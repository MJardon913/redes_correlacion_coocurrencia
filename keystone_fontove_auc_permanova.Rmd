...
nhood <- function(df, vec){
  ...
  while (i <= dim(df)[1] && df[i,1] != v){
    if (is.element(df[i,1] , vec) == TRUE && is.element(df[i,2] , vec) == TRUE  ){
      i <- i + 1
    } else { if (is.element(df[i,1] , vec) == TRUE) {
      nd <- c(nd, df[i,2])
    }
      if (is.element(df[i,2] , vec) == TRUE) {
      nd <- c(nd, df[i,1])
    }
      i <- i + 1  }
      }
...
distan <- function(df , nodes , u , v){
  ...
  i <- 0
  nd_0 <- c(u_0)
  while( is.element(u_1 , nd_0) == FALSE && i <=  length(nodes) ){
    nd_0 <- nhood(df , nd_0)
    i <- i + 1  
    }
